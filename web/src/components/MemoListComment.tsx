...
const MemoListComment: React.FC<Props> = (props: Props) => {
  const { showCreator, postId } = props;
  ...
  const { memos, isFetching } = memoStore.state2;
  ...
  useEffect(() => {
    memoStore
      .fetchMemosByPostId(undefined, undefined, postId)
      .then((fetchedMemos) => {
  ...
  const handleFetchMoreClick = async () => {
    try {
      const fetchedMemos = await memoStore.fetchMemosByPostId(DEFAULT_MEMO_LIMIT, memos.length, postId);
      if (fetchedMemos.length < DEFAULT_MEMO_LIMIT) {
        setIsComplete(true);
      } else {
        setIsComplete(false);
...
export default MemoListComment;
